        ...
        const hist = {
            ...
            render: () => {
                ...
                // Mostrar últimos 5 por defecto, o todos con scroll
                state.db.estimates.forEach(est => {
                    const el = document.createElement('div');
                    el.className = 'history-item';
                    if(state.selectedIds.includes(est.id)) el.classList.add('selected');
                    
                    el.innerHTML = `
                        <div style="display:flex; justify-content:space-between;">
                            <h4>${est.client}</h4>
                            <span style="font-weight:bold">$${est.total.toLocaleString()}</span>
                        </div>
                        <p>${est.date} - ${est.baseCalc.isML ? 'ML' : 'M²'}: ${est.baseCalc.area}</p>
                    `;

                    // Eventos Touch
                    el.addEventListener('touchstart', (e) => hist.handleTouchStart(e, est.id));
                    el.addEventListener('touchend', () => hist.handleTouchEnd());
                    el.addEventListener('click', () => hist.handleClick(est.id));
