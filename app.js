...
const storage = {
    load: () => {
        try {
            ...
            // Aplicar tema si settings ya está cargado
...
const hist = {
    ...
    printPDF: (index) => {
        ...
        doc.text(comppany.name || 'Presupuesto', left + 35, y + 8);
        ...
        const precioBruto = item.baseCalc.price + (item.baseCalc.discountAmount || 0);
...
const settings = {
    ...
    applyTheme: () => {
        if (!state.db.settings || !state.db.settings.theme) return;
        ...
        const companyName = document.getElementById('header-company-name');
        if (companyName) companyName.innerText = state.db.settings.company.name || 'Mi Empresa';

        if (state.db.settings.company.logo) {
            const img = document.getElementById('header-logo');
            if (img) img.src = state.db.settings.company.logo;
        }
    }
};
...
const ui = {
    ...
    enableAutocomplete(inputId, sourceMap) {
        ...
        input.addEventListener('focus', () => {
            ...
            list.style.border = '1px solid var(--border-color, #ccc)';
            ...
            input.parentElement.style.position = 'relative'; // Asegurar posicionamiento
...
// --- INICIALIZACIÓN ---
// Usamos DOMContentLoaded para que la app inicie correctamente
